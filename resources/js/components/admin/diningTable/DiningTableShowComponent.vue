	...
	<div class="col-12">
		<div class="db-card">
			...
			<div class="db-card-body" id="print">
				...
				<p class="text-center">
					<span class="block capitalize mt-4">{{
						diningTable.branch_name
					}}</span>
					<span class="block">{{ diningTable.branch_phone }}</span>
					<span class="block capitalize mb-6">{{
						diningTable.branch_address
					}}</span>
				</p>
				<img
					class="w-48 sm:w-60 mx-auto my-4"
				...
				<p class="text-center mt-2" v-if="diningTable.slug">
					<span class="block text-xs text-gray-500">Link:</span>
					<a
						:href="`${window.location.origin}/menu/${diningTable.slug}`"
						target="_blank"
...
export default {
	...
	mounted() {
		...
		this.$store
			.dispatch("diningTable/show", this.$route.params.id)
			.then((res) => {
