...
export default {
		...
		for (i = 0; i < routes.length; i++) {
			for (j = 0; j < permission.length; j++) {
				if (typeof routes[i].meta !== "undefined" && routes[i].meta) {
					if (
						typeof routes[i].meta.permissionUrl !== "undefined" &&
						routes[i].meta.permissionUrl
					) {
						...
						if (
							typeof routes[i].children !== "undefined" &&
							routes[i].children
						) {
		...
		singleElements?.forEach((singleElement) => {
			for (let i = 0; i < singleElement.childElementCount; i++) {
				singleElement?.children[i]?.addEventListener("click", function () {
					for (let a = 0; a < singleElement.childElementCount; a++)
						singleElement?.children[a]?.classList?.remove(addedClass);
	...
	handleSlide: function (id) {
		...
		if (targetElement.style.visibility == "visible") {
			...
			document
				.querySelectorAll(".table-filter-btn")
			...
			document
				.querySelectorAll(".table-filter-btn")
				.forEach((btn) => btn.classList.add("rotated"));
		}
	},
